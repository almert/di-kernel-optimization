...
class LaplacianKernel(Kernel):
    ...
    def __call__(self, X1, X2=None):
        if X2 is None:
            X2 = X1;
        X_rowdiff = tf.expand_dims(X1, 1) - tf.expand_dims(X2, 0)
...
class GaussianKernel(Kernel):
    ...
    def __call__(self, X1, X2=None):
        if X2 is None:
            X_inner = tf.matmul(X1,X1,transpose_b=True)
            X_norm = tf.diag_part(X_inner)
            X_dist_sq = X_norm+tf.reshape(X_norm, [-1,1])-2*X_inner
        else:
            X_dist_sq = tf.reshape(tf.reduce_sum(X1*X1,axis=1),(-1,1)) \
                -2*tf.matmul(X1,X2,transpose_b=True) \
                +tf.reduce_sum(X2*X2,axis=1);

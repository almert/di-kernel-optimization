...
for n in n_dims:
    ...
    kernel_func = RandomFourierLayer(n=n,
                                     kernel='rbf',
                                     gamma=gamma)
    kernel_func.initialize(Xtr,dtype=tf.float64)
    kernel_map = kernel_func.layer_func(x)

    objective = DI(kernel_map,y,rho=1e-4,normalize=False)
    ...
    print('Fitting a predictor on the optimized Fourier mapping.')        
    
    clf = TFClassifier(loss='logistic', 
                       alpha=0.)
    clf.fit(sess.run(kernel_map,
                     feed_dict={x: Xtr}),
            ytr)
    Accuracy = clf.score(sess.run(kernel_map,
                                  feed_dict={x: Xte}),
                         yte)
    sess.close()
